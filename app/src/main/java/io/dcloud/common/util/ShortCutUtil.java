...
public class ShortCutUtil {
    ...
    public static boolean showSettingsDialog(final IApp iApp, final String str, final Bitmap bitmap) {
        ...
        if (Build.BRAND.equalsIgnoreCase(MobilePhoneModel.QiKU)) {
            ...
            str2 = "VIVO手机使用流应用需先确定开启桌面快捷方式。请点击“前往设置权限”-软件管理-桌面快捷方式管理-“" + (BaseInfo.isForQihooHelper(iApp.getActivity()) ? "360手机助手" : "流应用") + "”-允许。如果软件管理界面中无“桌面快捷方式管理”项请升级到最新版ROM。";
        ...
        initDialogTheme.setMessage(str2).setTitle("创建桌面图标提醒").setPositiveButton(str3, new DialogInterface.OnClickListener() { // from class: io.dcloud.common.util.ShortCutUtil.3
    ...
    public static boolean showSettingsDialog(final Activity activity, final String str, final String str2, final String str3, final Bitmap bitmap) {
        ...
        initDialogTheme.setMessage("奇酷手机使用流应用需先确定权限开启，在设置-桌面设置中勾选快捷方式启用").setTitle("创建快捷方式提醒").setPositiveButton("前往设置", new DialogInterface.OnClickListener() { // from class: io.dcloud.common.util.ShortCutUtil.5
            @Override // android.content.DialogInterface.OnClickListener
            public void onClick(DialogInterface dialogInterface, int i) {
    ...
    public static void runShortCutToast(Activity activity) {
        if (ShortcutCreateUtil.canCreateShortcut(activity) && ShortcutCreateUtil.needToast(activity)) {
            Toast.makeText(activity, "桌面图标创建成功", Toast.LENGTH_SHORT).show();
            return;
        }
        if (ShortcutCreateUtil.isDisableShort(activity)) {
            if (ShortcutCreateUtil.isSystemLauncher(activity)) {
                Toast.makeText(activity, "当前系统不支持创建桌面图标", Toast.LENGTH_SHORT).show();
                return;
            } else {
                Toast.makeText(activity, "当前第三方的桌面不支持创建桌面图标", Toast.LENGTH_SHORT).show();
                return;
            }
        }
        ...
        Toast.makeText(activity, "请切换到桌面查看桌面图标是否创建", Toast.LENGTH_SHORT).show();
    }
