...
public final class FragmentManagerImpl extends FragmentManager implements LayoutInflater.Factory2 {
    ...
    static boolean shouldRunOnHWLayer(View view, AnimationOrAnimator animationOrAnimator) {
        return view != null && animationOrAnimator != null && Build.VERSION.SDK_INT >= 19 && view.getLayerType() == View.LAYER_TYPE_NONE && ViewCompat.hasOverlappingRendering(view) && modifiesAlpha(animationOrAnimator);
    }
    ...
    private static Animation.AnimationListener getAnimationListener(Animation animation) {
        if (Build.VERSION.SDK_INT >= 31) {
            // На Android 12+ не можна отримати, повертаємо null або кидаємо виключення
            return null;
        } else {
            try {
                @SuppressLint("SoonBlockedPrivateApi") Field declaredField = Animation.class.getDeclaredField("mListener");
                declaredField.setAccessible(true);
                return (Animation.AnimationListener) declaredField.get(animation);
            } catch (Exception e) {
                e.printStackTrace();
                return null;
            }
    ...
    void completeShowHideFragment(final Fragment fragment) {
        if (fragment.mView != null) {
            ...
            if (loadAnimation != null && loadAnimation.animator != null) {
                ...
                fragment.mView.setVisibility((!fragment.mHidden || fragment.isHideReplaced()) ? View.VISIBLE : View.GONE);
                if (fragment.isHideReplaced()) {
                    fragment.setHideReplaced(false);
                }
            }
    ...
    */
    ...
    public View onCreateView(View view, String str, Context context, AttributeSet attributeSet) {
        ...
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, new int[] {
                android.R.attr.name,
                android.R.attr.id,
                android.R.attr.tag
        });
        ...
        public void onAnimationEnd(Animation animation) {
            if (ViewCompat.isAttachedToWindow(this.mView) || Build.VERSION.SDK_INT >= 24) {
                this.mView.post(new Runnable() { // from class: android.support.v4.app.FragmentManagerImpl.AnimateOnHWLayerIfNeededListener.1
                    @Override // java.lang.Runnable
                    public void run() {
                        AnimateOnHWLayerIfNeededListener.this.mView.setLayerType(View.LAYER_TYPE_NONE, null);
                    }
                });
            } else {
