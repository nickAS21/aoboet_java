...
public class PrecomputedTextCompat implements Spannable {
    ...
    public static PrecomputedTextCompat create(CharSequence charSequence, Params params) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= 23) {
                StaticLayout.Builder.obtain(charSequence, 0, charSequence.length(), params.getTextPaint(), Integer.MAX_VALUE).setBreakStrategy(LineBreaker.BREAK_STRATEGY_SIMPLE).setHyphenationFrequency(Layout.HYPHENATION_FREQUENCY_NORMAL).setTextDirection(params.getTextDirection()).build();
    ...
    private int findParaIndex(int i) {
        ...
        while (true) {
            ...
            if (i2 < iArr.length) {
                ...
                throw new IndexOutOfBoundsException(new StringBuilder().append("pos must be less than ").append(this.mParagraphEnds[this.mParagraphEnds.length - 1]).append(", gave ").append(i).toString());
            }
        }
    }
