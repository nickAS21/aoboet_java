...
public class ViewCompat {
    ...
    static {
        ViewCompatImpl impl1;
        int i = Build.VERSION.SDK_INT;
        if (i >= 21) {
            impl1 = new LollipopViewCompatImpl();
        }
        if (i >= 19) {
            impl1 = new KitKatViewCompatImpl();
        }
        if (i >= 17) {
            impl1 = new JbMr1ViewCompatImpl();
        }
        if (i >= 16) {
            impl1 = new JBViewCompatImpl();
        }
        if (i >= 14) {
            impl1 = new ICSViewCompatImpl();
        }
        if (i >= 11) {
            impl1 = new HCViewCompatImpl();
        }
        if (i >= 9) {
            impl1 = new GBViewCompatImpl();
        } else if (i >= 7) {
            impl1 = new EclairMr1ViewCompatImpl();
        } else {
            impl1 = new BaseViewCompatImpl();
        }
        IMPL = impl1;
    }
