...
public final class TextViewCompat {
    ...
    public static void setCompoundDrawablesRelative(TextView textView, Drawable drawable, Drawable drawable2, Drawable drawable3, Drawable drawable4) {
        ...
        if (Build.VERSION.SDK_INT >= 17) {
            boolean z = textView.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL;
    ...
    public static class OreoCallback implements ActionMode.Callback {
        ...
        private boolean isSupportedActivity(ResolveInfo resolveInfo, Context context) {
            ...
            if (resolveInfo.activityInfo.exported) {
                return resolveInfo.activityInfo.permission == null || context.checkSelfPermission(resolveInfo.activityInfo.permission) == PackageManager.PERMISSION_GRANTED;
            }
            return false;
        }
    ...
    public static void setTextMetricsParams(TextView textView, PrecomputedTextCompat.Params params) {
        // API 18+ — встановлюємо напрям тексту
        if (Build.VERSION.SDK_INT >= 18) {
            textView.setTextDirection(getTextDirection(params.getTextDirection()));
        }

        // API < 23 — вручну копіюємо paint і масштабуємо, якщо потрібно
        if (Build.VERSION.SDK_INT < 23) {
            TextPaint textPaint = params.getTextPaint();
            float scaleX = textPaint.getTextScaleX();

            textView.getPaint().set(textPaint);
            if (scaleX == textView.getTextScaleX()) {
                // зламати кеш, щоб зміни спрацювали
                textView.setTextScaleX((scaleX / 2.0f) + 1.0f);
            }
            textView.setTextScaleX(scaleX);
        } else {
            // API 23+: можна напряму встановлювати стратегії розбиття рядків і гіпенізацію
            textView.getPaint().set(params.getTextPaint());
            textView.setBreakStrategy(LineBreaker.BREAK_STRATEGY_SIMPLE); // BREAK_STRATEGY_SIMPLE / HIGH_QUALITY / BALANCED
            textView.setHyphenationFrequency(Layout.HYPHENATION_FREQUENCY_NORMAL); // HYPHENATION_FREQUENCY_NONE / NORMAL / FULL
        }
    }
