...
public class IntKeyframeSet extends KeyframeSet {
    ...
    @Override // io.dcloud.nineoldandroids.animation.KeyframeSet
    /* renamed from: clone */
    public IntKeyframeSet m7clone() {
        ArrayList<Keyframe> arrayList = this.mKeyframes;
        int size = this.mKeyframes.size();
        Keyframe.IntKeyframe[] intKeyframeArr = new Keyframe.IntKeyframe[size];
        for (int i = 0; i < size; i++) {
            intKeyframeArr[i] = (Keyframe.IntKeyframe) arrayList.get(i).m6clone();
        }
    ...
    public int getIntValue(float f) {
        ...
        while (i < this.mNumKeyframes) {
            ...
            if (f < intKeyframe6.getFraction()) {
                ...
                return this.mEvaluator == null ? intValue6 + ((int) (fraction5 * (intValue7 - intValue6))) : ((Number) this.mEvaluator.evaluate(fraction5, Integer.valueOf(intValue6), Integer.valueOf(intKeyframe6.getIntValue()))).intValue();
            }
            i++;
